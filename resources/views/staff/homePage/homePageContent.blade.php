    ...
    <div class="d-flex">
        <div class="main-content p-4" style="width: 100%;">
            ...
            <div class="order-tabs mb-4">
                <a href="{{ url('/staffHomePage?status=all') }}" class="order-tab {{ $current_status == 'all' ? 'active' : '' }}">
                    All Orders
                    <span class="badge-count">{{ $counts['all'] }}</span>
                </a>
                <a href="{{ url('/staffHomePage?status=to_pay') }}" class="order-tab {{ $current_status == 'to_pay' ? 'active' : '' }}">
                    To Pay
                    @if($counts['to_pay'] > 0)
                        <span class="badge-count">{{ $counts['to_pay'] }}</span>
                    @endif
                </a>
                <a href="{{ url('/staffHomePage?status=to_ship') }}" class="order-tab {{ $current_status == 'to_ship' ? 'active' : '' }}">
                    To Ship
                    @if($counts['to_ship'] > 0)
                        <span class="badge-count">{{ $counts['to_ship'] }}</span>
                    @endif
                </a>
                <a href="{{ url('/staffHomePage?status=to_receive') }}" class="order-tab {{ $current_status == 'to_receive' ? 'active' : '' }}">
                    To Receive
                    @if($counts['to_receive'] > 0)
                        <span class="badge-count">{{ $counts['to_receive'] }}</span>
                    @endif
                </a>
                <a href="{{ url('/staffHomePage?status=completed') }}" class="order-tab {{ $current_status == 'completed' ? 'active' : '' }}">
                    Completed
                    @if($counts['completed'] > 0)
                        <span class="badge-count">{{ $counts['completed'] }}</span>
                    @endif
                </a>
                <a href="{{ url('/staffHomePage?status=cancelled') }}" class="order-tab {{ $current_status == 'cancelled' ? 'active' : '' }}">
                    Cancelled
                                    ...
                                    <div class="modal fade" id="updateModal{{ $order->order_id }}" tabindex="-1">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form method="post" action="{{ url('/staff/updateOrderStatus/'.$order->order_id) }}">
                                                    @csrf
